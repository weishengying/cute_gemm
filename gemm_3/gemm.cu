...
__global__ void gemm_shm_v3(const T *Aptr, const T *Bptr, T *Dptr, int m, int n, int k) {
    ...
    // Dreg -> shm -> reg -> global
    auto sC = make_tensor(sA(_, _).data(), SmemLayoutC{}); //(kMmaPM,kMmaPN)
    ...
    auto tCrC_r2sx = group_modes<1, 3>(tCrC_r2s);  // (CPY, CPY_M, CPY_N) --> (CPY_, CPY_MN) ((_2,(_2,_2)),(_4,_8))
    auto tCgC_s2gx = group_modes<1, 3>(tCgC_s2g);  // (CPY, CPY_M, CPY_N) --> (CPY_, CPY_MN) ((_8,_1),(_4,_8))
...
template <typename T>
void gemm_v3(const T *a, const T *b, T *c, int M, int N, int K) {
    auto BM = Int<128>{};
    auto BN = Int<256>{};
    auto BK = Int< 32>{};
    ...
    static constexpr int kMmaPM = kMmaVRepeatM * kMmaEURepeatM * get<0>(mma_atom_shape{});
    static constexpr int kMmaPN = kMmaVRepeatN * kMmaEURepeatN * get<1>(mma_atom_shape{});
    static constexpr int kMmaPK = kMmaVRepeatK * kMmaEURepeatK * get<2>(mma_atom_shape{});
    ...
    cudaFuncSetAttribute(gemm_shm_v3<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
    
    gemm_shm_v3<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
...
int main() {
    ...
    for (int j = 0; j < 10; j++) {
        ...
        float max_error = testF16F16GemmMaxError_V2<T>(gemm_v3, M, N, K);
