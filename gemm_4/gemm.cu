...
__global__ void gemm_v4_kernel(const T *Aptr, const T *Bptr, T *Dptr, int m, int n, int k) {
...
template <typename T>
void gemm_v4(const T *a, const T *b, T *c, int M, int N, int K) {
    auto BM = Int<128>{};
    auto BN = Int<256>{};
    auto BK = Int<32>{};
    auto KStage = Int<4>{};
    ...
    cudaFuncSetAttribute(gemm_v4_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
    
    gemm_v4_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
...
int main() {
    ...
    for (int j = 0; j < 5; j++) {
        ...
        float max_error = testF16F16GemmMaxError_V2<T>(gemm_v4, M, N, K);
